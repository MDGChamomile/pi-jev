...
test('session status counts calls separately from approved request attempts without retaining inputs', async () => {
  ...
  await runner.execute({}, catalog(), undefined, ctx());
  assert.equal(runner.getStatus().lastResult, 'invalid_input');
  await runner.execute(input(), catalog(), undefined, ctx(false));
  assert.equal(runner.getStatus().lastResult, 'declined');
  assert.equal(authCalls, 0);
  missingKey = true;
  await runner.execute(input(), catalog(), undefined, ctx());
...
test('session status handles busy calls and cancellation during authentication without counting a request', async () => {
  ...
  const pending = runner.execute(input(), catalog(), undefined, ctx());
  await started;
...
test('parent abort and shutdown cancel pending authentication, release the lock, and ignore late credentials', async () => {
  for (const cause of ['parent', 'shutdown']) {
    ...
    const first = runner.execute(input(), catalog(), controller.signal, ctx());
    await started;
    ...
    assert.equal((await runner.execute(input(), catalog(), undefined, ctx())).status, 'ok');
